...
HTML5 `<canvas>` 是一个 HTML 标签，你可以把它理解为一张**“空白的画布”**。
它本身不具备绘画能力，必须使用脚本（通常是 **JavaScript**）在这张画布上逐个像素地绘制图形、图像、动画或文本。

- **特点：** 基于位图（Bitmap）、立即模式渲染（Immediate Mode）、高性能。
    
- **用途：** 网页游戏、数据可视化图表（如 ECharts）、图片编辑器、粒子特效。
使用 Canvas 主要分三步：**建画布、拿画笔、画图形**。
入门
---
