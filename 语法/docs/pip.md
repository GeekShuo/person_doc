...
### 细说下pip -e
`pip install -e .` 是一个用于 **“以可编辑模式（Editable Mode）安装当前目录下的 Python 项目”** 的命令。

当你正在开发一个 Python 包，或者在修改一个开源项目的源代码时，这个命令非常有用

如果不加 `-e`，普通的安装过程如下：

1. pip 把你当前的代码**复制**到 Python 的安装目录。
    
2. 如果你修改了源代码，你的 Python 环境**不会**感知到变化。
    
3. 你必须重新运行 `pip install .` 才能让修改生效。
**使用了 `-e` 之后：** 你的代码依然留在当前文件夹里。当你修改代码并保存后，**无需重新安装**，下次运行程序或导入该包时，Python 会直接读取你修改后的代码。
#### 3. 常见使用场景

|场景|行为|结果|
|---|---|---|
|**开发自己的库**|在本地编写代码。|方便直接在测试脚本里 `import` 并实时调试。|
|**贡献开源代码**|`git clone` 某个项目。|边修改 Bug 边运行测试，改完即生效。|
|**多模块协作**|A 项目依赖 B 项目（都在本地）。|修改 B 的代码，A 会立即看到效果。|
那### 4. 注意事项

1. **不要移动文件夹**：因为安装的是“路径引用”，如果你移动或重命名了项目文件夹，这个安装就会失效，需要重新执行命令。
    
2. **卸载方式相同**：如果你想移除这个包，依然使用 `pip uninstall <项目名>`。
    
3. **配置文件要求**：你的项目中必须有有效的构建定义文件（如 `setup.py` 或现代的 `pyproject.toml`），否则 pip 会报错找不到项目元数据。
