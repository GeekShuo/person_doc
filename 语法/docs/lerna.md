...
`--skip-npm` [#](https://github.com/lerna/lerna/tree/main/commands/publish#--skip-npm) | 直接调用 [lerna version](lerna.md#version-修改版本号)
<!--rehype:className=style-list-arrow-->
...
列出本地程序包，也尊重所有可用的[过滤选项](lerna.md#过滤选项)
...
- `lerna changed` 支持 `lerna ls` 支持的所有[选项](lerna.md#选项-1)
...
接受所有[过滤选项](lerna.md#过滤选项)。`lerna clean` 不会从根 `node_modules` 目录中删除模块，即使您启用了 `--hoist` 选项
