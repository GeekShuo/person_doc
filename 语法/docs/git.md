...
Git Submodule 子模块
------
有的时候，我们在一个项目中可能会使用github上面另一个项目作为我们项目的一部分，如果直接clone，当你执行git clone时，这个子项目文件夹里自带了一个 .git目录。Git 发现这个目录后，会认为它是一个独立的项目。如果你直接用 git add 添加，主项目**只会记录这个文件夹的名字，而不会记录文件夹内部的具体文件内容**。这意味着，如果你把代码推送到 GitHub，别人下载你的主项目时，子项目文件夹会是空的。此时有两种方法。一、如果你不打算同步原作者以后的更新，只是想把这些代码直接变成你项目的一部分。那么删除子项目里面的.git文件夹，然后重新add。 方法二：将这个项目正式添加为子模块
### 添加子模块
