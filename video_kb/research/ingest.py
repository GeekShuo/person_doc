...
from config import NOTES_DIR, RAW_DIR, TRANSCRIPT_DIR
...
def ingest_text(r: SearchResult, topic: str) -> int:
    body = r.snippet or ""
    ...
    with open(os.path.join(NOTES_DIR, f"{vid}.md"), "w", encoding="utf-8") as f:
        f.write(md)
...
def ingest_video_result(r: SearchResult, topic: str,
    ...
    np = build_notes(tp, None, title=r.title)
