...
source: "https://onefly.top/zero2Leetcode/04_real_interviews/dewu/algo-20260829-b/index.html"
---
...
1. 如果新字符与原末尾字符不同，共有 25 种选择，新的连续段长度变为 1： \[f_{i,1}=25\cdot(f_{i-1,1}+f_{i-1,2}+f_{i-1,3})。\]
2. 如果新字符与原末尾字符相同，选择唯一，连续段长度增加 1： \[f_{i,2}=f_{i-1,1},\qquad f_{i,3}=f_{i-1,2}。\] 原末尾长度为 3 时不能再添加相同字符，否则会产生四个连续相同字符。
