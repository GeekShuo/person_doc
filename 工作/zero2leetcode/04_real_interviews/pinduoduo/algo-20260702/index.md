---
title: "拼多多算法岗 7.2"
source: "https://onefly.top/zero2Leetcode/04_real_interviews/pinduoduo/algo-20260702/index.html"
---
...
**考试时间**：2026年7月2日 **考试岗位**：大模型算法岗 **难度评级**：中等
...
**时间复杂度**：$O(mn)$，每个格子恰好访问一次。 **空间复杂度**：$O(mn)$，存储矩阵。
...
**时间复杂度**：$O(n \cdot E)$，$E$ 为监视关系总数，每轮剥离至少消除一个守卫。 **空间复杂度**：$O(n + E)$。
...
**时间复杂度**：$O(n)$，每个房间入队出队各一次，每个环完整遍历一次。 **空间复杂度**：$O(n)$。
...
**时间复杂度**：$O(N^2 D)$，主要开销是相似度矩阵的矩阵乘法。 **空间复杂度**：$O(N^2)$，存储 logits 矩阵。
